...
geo@data <- left_join(geo@data, obesity, by = c("name" = "Entity"))

bins <- c(0, 2, 5, 10, 15, 20, 25, 30, Inf)
pal <- colorBin("YlOrRd", domain = geo@data$percentBMI30.y, bins = bins)

leaflet(geo) %>%
  setView(lng = 0, lat = 20, zoom = 2) %>%
  addPolygons(fillColor = ~pal(percentBMI30.y),
              weight    = 2,
              opacity   = 1,
              dashArray = "3",)
```
