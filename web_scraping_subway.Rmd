...
addressParser <- function(url) {                                   #Creating a function for parsing addresses on the Subway website
  links <- url %>%                                      # Read in the html url and referring it to links in the function
  ...
  if (length(links) == 0) {                                         # If the link has no elements (changed from "link" to "links")
    
  } else {
    c(lapply(links, addressParser), recurisve = TRUE)      # Otherwise keep repeating the loop until it does, and give a list that does not have nested lists
